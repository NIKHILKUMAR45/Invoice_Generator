...
function App() {
  ...
  const handleProductChange = (value, index) => {
    ...
    const sgstAmount = (
      (selectedProduct.sgst * selectedProduct.rate) /
      100
    ).toFixed(2);
    const cgstAmount = (
      (selectedProduct.cgst * selectedProduct.rate) /
      100
    ).toFixed(2);
    const amount = (
      parseFloat(selectedProduct.rate) +
  ...
  useEffect(() => {
    const { totalAmount, totalDiscAmt, totalTaxableINR, cgstTotal, sgstTotal } =
      noOfRows.reduce(
        (accumulator, row) => ({
          totalAmount: accumulator.totalAmount + (parseFloat(row.amount) || 0),
          totalDiscAmt:
            accumulator.totalDiscAmt + (parseFloat(row.discountAmt) || 0),
          totalTaxableINR:
            accumulator.totalTaxableINR + (parseFloat(row.taxableINR) || 0),
  ...
  return (
    <>
      <div className="App">
        <h1>Invoice Generating System</h1>
              ...
              <tbody>
                {noOfRows.map((row, i) => (
                  <tr key={i}>
                    ...
                    <td><button className='remove' onClick={() => removeRow(i)}>
                      X</button></td>
                  </tr>
                ))}
        ...
        <div className="buttonWrapper">
          ...
          <button onClick={handleSnapshot}>Generate Invoice</button>
